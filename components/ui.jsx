...
function TabBar({ tabs, active, onChange, glowing = new Set() }) { return <div style={{ display: "flex", gap: 2, overflowX: "auto", padding: "0 12px", borderBottom: `1px solid ${P.border}`, background: P.bg, position: "sticky", top: 0, zIndex: 100 }}>{tabs.map(t => <button key={t} onClick={() => onChange(t)} className={glowing.has(t) && active !== t ? "tab-pulse" : ""} style={{ padding: "10px 14px", fontSize: 13, fontWeight: active === t ? 700 : 500, color: active === t ? P.accent : glowing.has(t) && active !== t ? P.accent : P.textMuted, background: "none", border: "none", borderBottom: active === t ? `2px solid ${P.accent}` : "2px solid transparent", cursor: "pointer", whiteSpace: "nowrap", fontFamily: "inherit" }}>{t}</button>)}</div>; }
