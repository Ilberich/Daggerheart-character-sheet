                      ...
                      <div key={card.name} onClick={() => !isDisabled && toggleCard(domain, card.name)}
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                          ...
                          <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 8 }}>
                            ...
                            {(() => { const cd = costDisplayMulti(card); return <div style={{ fontSize: 10, fontWeight: 700, fontFamily: mono, color: cd.color, marginTop: 2 }}>USE: {cd.text}</div>; })()}
                          </div>
                        </div>
                ...
                <div key={key} style={{ marginBottom: 8, borderRadius: 10, border: `1px solid ${domColor}55`, background: domColor + "0d", overflow: "hidden" }}>
                  <div style={{ padding: "8px 12px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {(() => { const cd = costDisplayMulti(card); return <span style={{ fontSize: 11, fontWeight: 700, fontFamily: mono, color: cd.color }}>⚡{cd.text}</span>; })()}
                    </div>
                    <div style={{ fontSize: 11, lineHeight: 1.65, color: P.textMuted, whiteSpace: "pre-line", marginTop: 6, paddingLeft: 16 }}>{card.text}</div>
                    {card.optionalCost && <div style={{ fontSize: 10, fontWeight: 700, fontFamily: mono, color: card.optionalCost.type === "hope" ? P.hope : P.stress, marginTop: 4, marginLeft: 16, padding: "3px 8px", border: `1px dashed ${card.optionalCost.type === "hope" ? P.hope + "66" : P.stress + "66"}`, borderRadius: 6, display: "inline-block" }}>⚡ {card.optionalCost.amount} {card.optionalCost.type === "hope" ? "Hope" : "Stress"} → {card.optionalCost.label}</div>}
                  </div>
                </div>
              );
            })}
